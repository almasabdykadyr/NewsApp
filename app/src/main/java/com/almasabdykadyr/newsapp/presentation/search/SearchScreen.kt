...
@Composable
fun SearchScreen(
    state: SearchState, event: (SearchEvent) -> Unit, navigation: (String) -> Unit
) {
    ...
    Column(
        ...
        SearchBar(text = state.searchQuery,
            readOnly = false,
            onValueChanged = { event.invoke(SearchEvent.UpdateSearchQuery(it)) },
            onSearch = { event.invoke(SearchEvent.SearchNews) }
        )
